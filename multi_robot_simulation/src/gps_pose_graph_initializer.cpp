...
GpsPoseGraphInitializer::GpsPoseGraphInitializer(
  ...
  ROS_INFO_STREAM("[GPS Initializer] Ready to initialize agent " << agent_id_);

  // Waypoints for initialization in ground truth frame (plus, landing)
  std::vector<Eigen::Vector3d> waypoints(9, odometry_.position_W);
  waypoints[1] += Eigen::Vector3d(delta_d, 0.0, 0.0);
  waypoints[2] += Eigen::Vector3d(delta_d, delta_d, 0.0);
  waypoints[3] += Eigen::Vector3d(0.0, delta_d, 0.0);
  waypoints[4] += Eigen::Vector3d(-delta_d, -delta_d, 0.0);
  waypoints[5] += Eigen::Vector3d(-delta_d, 0.0, 0.0);
  waypoints[6] += Eigen::Vector3d(-delta_d, -delta_d, 0.0);
  waypoints[7] += Eigen::Vector3d(0.0, -delta_d, 0.0);
