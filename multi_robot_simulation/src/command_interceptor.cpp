...
CommandInterceptor::CommandInterceptor(const ros::NodeHandle &nh,
                             ...
                             const int agent_id)
        : nh_(nh), nh_private_(nh_private), agent_id_(agent_id),
          T_C_O_(Eigen::Matrix4d::Identity()),
          has_valid_transformation_(false) {

  // Get parameters
  std::string ns(
          "/command_interceptor_node_" + std::to_string(agent_id_) + "/");
  CHECK(nh_private_.getParam(ns + "controller_frame_id", controller_frame_id_))
        << "Controller reference frame name not specified (param: "
        << ns << "controller_frame_id)";
  CHECK(nh_private_.getParam(ns + "state_frame_id", state_frame_id_))
        << "State reference frame name not specified (param: "
        << ns << "state_frame_id)";
  ...
  trajectory_cmd_sub_ = nh_.subscribe(
          "command/trajectory_odom", 10,
...
void CommandInterceptor::commandTrajectoryCallback(const
  ...
  try {
    ...
    T_O_S.block<3,1>(0,3) = Eigen::Vector3d(
            tf_O_S.getOrigin().getX(),
            tf_O_S.getOrigin().getY(),
    ...
    T_C_S.block<3,1>(0,3) = Eigen::Vector3d(
            tf_C_S.getOrigin().getX(),
            tf_C_S.getOrigin().getY(),
  ...
  for(size_t i = 0; i < traj_msg->points.size(); ++i) {
    ...
    Eigen::Vector4d position(
            T_C_O_ * Eigen::Vector4d(
                    traj_msg->points[i].transforms[0].translation.x,
