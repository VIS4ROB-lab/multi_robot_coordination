...
namespace pointcloud_filter {
  ...
  PointcloudFilter::PointcloudFilter(const ros::NodeHandle& nh,
                                     ...
                                     const int agent_id)
       : nh_(nh), nh_private_(nh_private), num_agents_(1), agent_id_(agent_id),
         world_frame_("world"), agent_frame_("agent"), ball_radius_(2.0),
         pointcloud_topic_("/dense_stereo/pointcloud") {

    // Get parameters
    std::string ns(
            "pointcloud_filter_node_" + std::to_string(agent_id_) + "/");
    ...
    for(uint64_t id = 0; id < num_agents_; ++id) {
      ros::Subscriber odom_sub =
              nh_.subscribe<nav_msgs::Odometry>(
                      ...
                      boost::bind(
                              &PointcloudFilter::odometryCallback,
                              this, _1, id));
      ...
      ros::Subscriber transf_map_odom =
              nh_.subscribe<geometry_msgs::TransformStamped>(
                      ...
                      boost::bind(
                              &PointcloudFilter::odomToMapCallback,
                              this, _1, id));
      ...
      ros::Subscriber transf_world_map =
              nh_.subscribe<geometry_msgs::TransformStamped>(
                      ...
                      boost::bind(
                              &PointcloudFilter::mapToWorldCallback,
                              this, _1, id));
  ...
  void PointcloudFilter::pointCloudCallback(
    ...
    if (!tf_listener_.canTransform(world_frame_, pcl_msg->header.frame_id,
      ...
      ROS_WARN("[PCL Filter] Using latest TF transform instead of timestamp "
               "match.");
    }
    ...
    try {
      ...
      ROS_ERROR_STREAM("[PCL Filter] Error getting TF transform from sensor "
                       "data: " << ex.what());
    ...
    for(int id = 0; id < num_agents_; ++id) {
      ...
      if(!getPositionAgentId(id, position)) {
        ROS_ERROR_STREAM("[PCL Filter] Position of agent " << id 
    ...
    tf::transformTFToEigen(tf::Transform(
            tf_transform_stamped.inverse().getRotation(),
            tf_transform_stamped.inverse().getOrigin()), affine_tranf);
  ...
  void PointcloudFilter::odometryCallback(
    ...
    ROS_INFO_STREAM_ONCE("[PCL Filter] Got first odometry message for "
                         "agent " << agent_id);
  }
  ...
  void PointcloudFilter::odomToMapCallback(
    ...
    ROS_INFO_STREAM_ONCE("[PCL Filter] Got first odom to map "
  ...
  void PointcloudFilter::mapToWorldCallback(
    ...
    ROS_INFO_STREAM_ONCE("[PCL Filter] Got first map to odom "
                         "transformation for agent " << agent_id);
  }
  ...
  bool PointcloudFilter::getPositionAgentId(const int agent_id,
    ...
    try {
      ...
      if(!transformations_initialized_W_M_[agent_id] ||
        ...
        ROS_WARN_STREAM_THROTTLE(5, "[PCL Filter] For agent " << agent_id
