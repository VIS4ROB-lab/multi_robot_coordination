...
class VoxbloxValidityCheckerMultiAgentCompoundState
 ...
 public:
  VoxbloxValidityCheckerMultiAgentCompoundState(
      ...
      double safety_factor, const int num_agents,
  ...
  virtual bool isValid(const base::State *state) const {
    ...
    for (int id = 0; id < num_agents_; ++id) {
      Eigen::Vector3d robot_position(
          Eigen::Vector3d(state->as<ompl::base::CompoundState>()
 ...
 public:
  ...
  virtual bool isValid(const base::State *state) const {
    ...
    for (int id = 1; id < check_paths_.size(); ++id) {
      ...
      for (std::vector<Eigen::Vector3d>::iterator it = path_master.begin();
           it < path_master.end(); ++it) {
...
class TsdfVoxbloxValidityCheckerMultiAgentCompoundState
    : public VoxbloxValidityCheckerMultiAgentCompoundState<voxblox::TsdfVoxel> {
 public:
  TsdfVoxbloxValidityCheckerMultiAgentCompoundState(
      ...
      voxblox::Layer<voxblox::TsdfVoxel> *tsdf_layer)
      : VoxbloxValidityCheckerMultiAgentCompoundState(
            space_info, robot_radius, safety_factor, num_agents, tsdf_layer),
        treat_unknown_as_occupied_(false) {}
...
class TsdfVoxbloxValidityCheckerHierarchicalMultiAgent
    : public VoxbloxValidityCheckerHierarchicalMultiAgent<voxblox::TsdfVoxel> {
 public:
  TsdfVoxbloxValidityCheckerHierarchicalMultiAgent(
      ...
      voxblox::Layer<voxblox::TsdfVoxel> *tsdf_layer)
      : VoxbloxValidityCheckerHierarchicalMultiAgent(
            space_info, robot_radius, safety_factor, check_paths, tsdf_layer),
        treat_unknown_as_occupied_(false) {}
...
class EsdfVoxbloxValidityCheckerHierarchicalMultiAgent
    : public VoxbloxValidityCheckerHierarchicalMultiAgent<voxblox::EsdfVoxel> {
 public:
        ...
        if (id != 0) {
          for (int j = 0; j < id; ++j) {
            ...
            for (std::vector<Eigen::Vector3d>::iterator it = points[j].begin();
                 it < points[j].end(); ++it) {
              if (((*it) - pos).norm() < validity_checker_->safety_distance()) {
                return false;
              }
            }
          }
